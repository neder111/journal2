    ...
    for i in range(12):
        ...
        for j in range(10):
            ...
            if j <9:
                ...
                print("TCP/UDP traffic between %s and h2" % (src))
                src.cmd("iperf -p 5050 -c 10.0.0.2")
                src.cmd("iperf -p 5051 -u -c 10.0.0.2")
            else:
                print("generating ICMP traffic between %s and h%s and TCP/UDP traffic between %s and h1" % (src,((dst.split('.'))[3]),src))
                src.cmd("ping {} -c 100".format(dst))
                src.cmd("iperf -p 5050 -c 10.0.0.1")
            ...
            print("%s Downloading index.html from h2" % src)
            src.cmd("wget http://10.0.0.2/index.html")
            print("%s Downloading test.zip from h2" % src)
            src.cmd("wget http://10.0.0.2/test.zip")
        
        h1.cmd("rm -f *.* /home/mininet/Downloads")
        h2.cmd("rm -f *.* /home/mininet/Downloads")
