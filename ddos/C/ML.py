...
class MachineLearning():
    ...
    def Confusion_matrix(self):
        ...
        prec=precision_score(self.y_flow_test, self.y_flow_pred)
        rec= recall_score(self.y_flow_test, self.y_flow_pred)
        f1= f1_score(self.y_flow_test, self.y_flow_pred)
        aucc=roc_auc_score(self.y_flow_test, self.y_flow_pred)
        FPR, TPR, cutoffs = metrics.roc_curve(self.y_flow_test, self.y_flow_pred,pos_label=1)  
        far=FPR
        print(" success accuracy = {0:.2f} %".format(acc*100))
        print("succes precision = {0:.2f} %".format(prec*100))
        print("succes recall = {0:.2f} %".format(rec*100))
        print("succes F1-score = {0:.2f} %".format(f1*100))
        print("succes AUC-ROC = {0:.2f} %".format(aucc*100))
        print("succes False  Alarm Rate  = {0:.2f} %".format(FPR*100))
        fail = 1.0 - acc
